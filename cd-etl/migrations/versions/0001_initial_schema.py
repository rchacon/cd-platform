...
Revision ID: 0001
Revises:
Create Date: 2026-07-28 22:14:56.456751

Ports the schema previously defined in the repo root's init.sql (retired --
see rchacon/cd-platform#5) verbatim, as one op.execute() per top-level
statement so each piece can still be read/reviewed on its own.
"""
...
def upgrade() -> None:
    ...
    op.execute(
        """
        -- Seeds the 119th Congress for initial deployment. The ETL
        -- process is responsible for discovering, inserting, and
        -- updating future Congress entries from the authoritative
...
def downgrade() -> None:
    ...
    op.execute("DROP FUNCTION current_congress()")
    op.execute("DROP TABLE member_terms")
    op.execute("DROP TABLE members")
    op.execute("DROP TABLE congresses")
    op.execute("DROP TYPE chamber_type")
